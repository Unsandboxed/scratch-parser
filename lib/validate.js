...
var isUbpProject = function (input) {
    ...
    if (hasOwn.call(input.meta, 'ubpVersion') && Number.isInteger(input.meta.ubpVersion)) {
        return true;
    }
    return input.meta.ubp === 1;
};

var validate = function (isSprite, input, callback) {
    var isUbp = !isSprite && isUbpProject(input);

    var validateUbp = null;
    var isValidUbp = false;
    if (isUbp) {
        validateUbp = ajv.compile(ubpSchema);
        isValidUbp = validateUbp(input);
        if (isValidUbp) {
            input.projectVersion = 3;
            input.projectFormat = 'ubp';
    ...
    if (isValidSb2) {
        ...
        input.projectFormat = 'sb2';
        return callback(null, input);
    }

    var validationErrors = {
        validationError: 'Could not parse as a valid SB2, SB3, or UBP project.',
        ubpErrors: validateUbp ? validateUbp.errors : null,
